...
better result, achieving an F1 score of \textcolor{red}{(???)}.
\end{abstract}
...
\section{\textcolor{red}{Notes for myself}}
Distribution of data
First tokenize on full words, but then used BERT tokenizer with Multilanguage.
Capitalization is notable.
Bert split words embedding not great, no benefit from using it.
Combining back to words is good since we cluster on word level
Cluster 1 stage, then 2 stage, and then NN
Fpan F1 not a good idea to even make even if it's the main metric, because our model already does not perform well.
NE vs Non NE is really good.
...
Then we evaluated the clusters as a baseline for our model. We used the F1 score to measure the perfomance. \textcolor{red}{F1 = !!!INSERT F1!!1}
